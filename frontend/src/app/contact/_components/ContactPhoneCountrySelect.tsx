...
function CountryFlag({ emojiFlag }: { emojiFlag: string }) {
  return (
    <span className="contact-phone-flag" aria-hidden>
      {emojiFlag}
    </span>
  );
}

export function ContactPhoneCountrySelect({ valueIso2, onChange }: Props) {
  ...
  useEffect(() => {
    ...
    const updateMenuBox = () => {
      ...
      setMenuBox({
        top: rect.bottom + 6,
        left: rect.left,
        width: Math.min(320, Math.max(rect.width, 260)),
  ...
  return (
    <div className="contact-phone-cc-dd">
      <button
        ref={triggerRef}
        type="button"
        className={`contact-phone-cc-trigger${selectedCountry ? "" : " contact-phone-cc-trigger--placeholder"}`}
      ...
      >
        {selectedCountry ? (
          <span className="contact-phone-cc-value">
            <CountryFlag emojiFlag={selectedCountry.emojiFlag} />
            <span>{selectedCountry.dialCode}</span>
          </span>
        ) : (
          <span className="contact-phone-cc-value">+ Code</span>
        )}
        <span className={`contact-phone-cc-chevron${open ? " contact-phone-cc-chevron--open" : ""}`} aria-hidden />
      </button>
      {open && menuBox && typeof document !== "undefined"
        ? createPortal(
            <div
              ref={menuRef}
              className="contact-phone-menu"
              role="listbox"
            ...
            >
              <div className="contact-phone-search-wrap">
                <input
                  ref={searchInputRef}
                  className="contact-phone-search-input"
                  type="text"
              ...
              <div
                className="contact-phone-menu-list"
                data-lenis-prevent
                data-lenis-prevent-wheel
                data-lenis-prevent-touch
              >
                {filteredOptions.length === 0 ? (
                  <div className="contact-phone-empty">No countries found.</div>
                ) : (
                  filteredOptions.map((option, index) => (
                    <button
                      ...
                      className={`contact-phone-menu-option${index === activeIndex ? " contact-phone-menu-option--active" : ""}`}
                      onMouseEnter={() => setActiveIndex(index)}
                      onClick={() => choose(index)}
                    >
                      <span className="contact-phone-cc-option">
                        <CountryFlag emojiFlag={option.emojiFlag} />
                        <span>{option.dialCode}</span>
                        <span className="contact-phone-cc-country">{option.countryName}</span>
                      </span>
                    </button>
                  ))
                )}
